...
type configureWebpackFunction = (config: WebpackConfiguration) => void;
type configureWebpackObject = WebpackConfiguration;
type Favicon = {
  favicon: {
    src: string;
    touchicon: string;
...
interface Permalinks {
   ...
   */
  slugify: Function | { use: string; options: object };
}
